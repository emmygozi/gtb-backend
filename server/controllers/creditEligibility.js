...
class CreditEligbility {
  static async eligible(req, res) {
    ...
    const accountDetails = await pool.query(accountQuery);
    const {
      age, returnedchequecount, taxid, minmonthlodgement, minqtlodgement, classificationname,
      yearlyturnover
    ...
    const overallrating = (agerating + returnedchequerating
        + classrating + taxidrating
        + yearlyturnoverrating + minmtlodgementrating + minqtlodgementrating) / 7;
    const ratings = {
      agerating,
  ...
  static async allEligible(req, res) {
    ...
    for (let i = 0; i < accountDetails.rows.length; i++) {
      const {
        accountno, accountname, age, returnedchequecount,
        taxid, minmonthlodgement, minqtlodgement, classificationname,
        yearlyturnover
      ...
      const overallrating = [];
      overallrating.push((agerating + returnedchequerating
        + classrating + taxidrating + yearlyturnoverrating
        + minmtlodgementrating + minqtlodgementrating) / 7);
...
export default CreditEligbility;
